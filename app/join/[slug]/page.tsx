...
export default function JoinPage({ params }: JoinPageProps) {
  ...
  const [quiz, setQuiz] = useState<{ title: string; description: string; type: string; questions?: number; duration?: string; price: number } | null>(null);
  
  // Обрабатываем асинхронные params
  useEffect(() => {
    params.then(({ slug: resolvedSlug }) => {
      setSlug(resolvedSlug);
      const quizData = getQuizBySlug(resolvedSlug);
      if (quizData) {
        setQuiz(quizData);
      }
    });
